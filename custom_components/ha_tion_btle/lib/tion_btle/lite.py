...
class TionLite(TionLiteFamily):
    ...
    def _decode_response(self, response: bytearray):
        ...
        try:
            ...
            self._electronic_work_time = (
                int.from_bytes(response[8:11], byteorder="little", signed=False) / 86400
            )  # days
            self._filter_remain = (
                int.from_bytes(response[16:20], byteorder="little", signed=False)
                / 86400
            )  # days
            self._device_work_time = (
            ...
            raise TionException(
                ...
                "Got bad response from Tion '%s': %s while parsing"
                % (response, str(e)),
            )
