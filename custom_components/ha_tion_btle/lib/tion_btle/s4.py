...
class TionS4(TionLiteFamily):
    ...
    def _encode_request(self, request: dict) -> bytearray:
        ...
        return bytearray(
            [0x00, 0x17, 0x00, self.MAGIC_NUMBER, self.random]
            + self.SET_PARAMS
            + self.random4
            + self.random4
            + [
                encode_state(),
                0x00,
                self._encode_mode(request["mode"]),
                int(request["heater_temp"]),
                int(request["fan_speed"]),
            ]
            + list(sign.to_bytes(2, byteorder="little"))
            + self.CRC
        )
    ...
    def command_getStatus(self) -> bytearray:
        return bytearray(
            [TionLiteFamily.SINGLE_PACKET_ID, 0x10, 0x00, self.MAGIC_NUMBER, 0xA1]
            + self.REQUEST_PARAMS
            + self.random4
            + self.random4
            + self.CRC
