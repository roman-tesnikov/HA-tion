...
PLATFORM_SCHEMA = PLATFORM_SCHEMA.extend(
    {
        ...
        vol.Optional(CONF_KEEP_ALIVE, default=30): vol.All(
...
class TionClimateEntity(ClimateEntity, CoordinatorEntity):
    ...
    def __init__(self, hass: HomeAssistant, instance: TionInstance):
        ...
        self._attr_preset_modes = list(self._attr_preset_modes)
    ...
    async def async_set_hvac_mode(self, hvac_mode: HVACMode):
        """Set hvac mode."""
        _LOGGER.info(
            "Need to set mode to %s, current mode is %s", hvac_mode, self.hvac_mode
        )
        if self.hvac_mode == hvac_mode:
            # Do nothing if mode is same
            _LOGGER.debug(
                f"{self.name} is asked for mode {hvac_mode}, but it is already in {self.hvac_mode}. Do "
                f"nothing."
            )
            pass
            ...
            was_fan_only = self.hvac_mode == HVACMode.FAN_ONLY
            await self._async_set_state(heater=True, is_on=True)
            if was_fan_only:
                await self.async_set_temperature(
                    **{ATTR_TEMPERATURE: saved_target_temp}
                )
    ...
    async def async_set_preset_mode(self, preset_mode: str):
        ...
        actions = []
        _LOGGER.debug(
            "Going to change preset mode from %s to %s", self.preset_mode, preset_mode
        )
        if preset_mode == PRESET_AWAY and self.preset_mode != PRESET_AWAY:
            _LOGGER.info(
                "Going to AWAY mode. Will save target temperature %s",
        ...
        if (
            ...
            and self._saved_target_temp
        ):
            # retuning from away mode
            _LOGGER.info(
                "Returning from AWAY mode: will set saved temperature %s",
                self._saved_target_temp,
            )
            actions.append(
                [self._async_set_state, {"heater_temp": self._saved_target_temp}]
            )
            self._saved_target_temp = None
        ...
        if preset_mode == PRESET_SLEEP and self.preset_mode != PRESET_SLEEP:
            ...
            actions.append(
                [
                    self.async_set_fan_mode,
                    {"fan_mode": min(int(self.fan_mode), self.sleep_max_fan_mode)},
                ]
            )
        ...
        if self.preset_mode in [PRESET_BOOST, PRESET_SLEEP] and preset_mode not in [
            PRESET_BOOST,
            PRESET_SLEEP,
        ]:
    ...
    async def async_set_fan_mode(self, fan_mode):
        if self.preset_mode == PRESET_SLEEP:
            if int(fan_mode) > self.sleep_max_fan_mode:
                _LOGGER.info(
                    "Fan speed %s was required, but I'm in SLEEP mode, so it should not be greater than %d",
                    self.sleep_max_fan_mode,
                )
                fan_mode = self.sleep_max_fan_mode

        if (
            self.preset_mode == PRESET_BOOST and self._is_boost
        ) and fan_mode != self.boost_fan_mode:
            _LOGGER.debug(
                "I'm in boost mode. Will ignore requested fan speed %s" % fan_mode
            )
    ...
    def _handle_coordinator_update(self) -> None:
        self._get_current_state()
        if int(self.fan_mode) != self.boost_fan_mode and (
            self._is_boost or self.preset_mode == PRESET_BOOST
        ):
            _LOGGER.warning(
                f"I'm in boost mode, but current speed {self.fan_mode} is not equal boost speed "
