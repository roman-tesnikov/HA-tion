...
from .lib.tion_btle.tion import Tion
...
class TionFlow:
    ...
    def __add_value_from_saved_settings(config: dict, key: str) -> dict:
        ...
        try:
            value = (
                config[key].seconds
                if isinstance(config[key], datetime.timedelta)
                else config[key]
            )
    ...
    def get_tion(
        ...
        if btle_device is None:
            ...
            _LOGGER.error("get_tion: %s", message)
...
class TionConfigFlow(TionFlow, config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, input=None):
        ...
        return self.async_show_form(
            step_id="user", data_schema=self.get_schema(TION_SCHEMA)
        )
    ...
    async def async_step_pair(self, input):
        ...
        try:
            ...
            _LOGGER.error(
                "Cannot pair and get data. Data is %s, result is %s; %s: %s",
                self._data,
                result,
                type(e).__name__,
                str(e),
...
class TionOptionsFlowHandler(TionConfigFlow, config_entries.OptionsFlow):
    ...
    async def async_step_init(self, input=None):
        if input is not None:
            ...
            return self.async_show_form(
                step_id="init", data_schema=self.get_schema(TION_OPTIONS_SCHEMA)
            )
